...
## 모집 부분(레일즈 개발자 2명, iOS/안드로이드 개발자 1명)

> Ruby on Rails 에 기술적으로 능숙한 경력개발자면 좋지만 패션업계에 관심있고 열정이 있는 분을 더 환영합니다.

- Ruby on Rails 개발자 경력 1명 (Ruby on Rails 개발 1년차 경력 이상 개발자)
- Ruby on Rails 개발자 신입 1명 (Ruby 는 경험없는 타언어 경험자를 위하여 1달간 교육기간 있음)
- iOS/Android 개발자 1명(구글 스토어 앱 등록 경험자)

신입/경력 공통적으로 1달간 수습기간후(급여는 동일) 정직원 채용합니다. 
...
- 연봉이 포함된 자율형식의 이력서를 openhr@stripes.co.kr로 보내주세요.
- 경력자는 Github 계정이나 이메일 첨부로 평가 가능한 자신만의 개인 프로젝트 소스코드를 포트폴리오로 제출해서 면접을 진행 합니다.
...
```
1차: 제출한 서류를 바탕으로 내부 평가 전형후 통지
2차: 면접으로 제출 경력과 포르폴리오 평가후 필기+실기 코딩 테스트
3차: 임원진/HR 면접
```
